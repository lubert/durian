...
- (id)initWithURL:(NSURL*)urlToOpen
{
    ...
    if (str)
        mSndFileRef = sf_open(str, SFM_READ, &mSF_Info);
...
- (void)close
{
    ...
    if (mTmplibSampleRateOutBuf) {
        free(mTmplibSampleRateOutBuf);
        mTmplibSampleRateOutBuf = NULL;
    }
    if (mTmpSndFileSourceData) {
        free(mTmpSndFileSourceData);
        mTmpSndFileSourceData = NULL;
    }
    if (mCoreAudioConverterRef) {
        AudioConverterDispose(mCoreAudioConverterRef);
        mCoreAudioConverterRef = NULL;
    }
            ...
            ForBuffer:(int)bufIdx
{
    ...
    if (maxBufSize > SF_COUNT_MAX)
        maxBufSize = INT32_MAX;
...
{
    if (nbFramesToRead > TMP_SRC_BUFFER_SIZE)
        nbFramesToRead = TMP_SRC_BUFFER_SIZE;
