...
+ (bool)isFormatSupported:(NSURL*)fileURL
{
    ...
    if ([[self supportedFileExtensions] containsObject:fileExtension])
        isSupported = true;
    else
        isSupported = false;
...
- (id)initWithURL:(NSURL*)urlToOpen
{
    ...
    if (mInputStreamFormat.mBitsPerChannel == 0) {
        if (mInputStreamFormat.mFormatID == kAudioFormatAppleLossless) {
            ...
            mBitDepth = 16; // Assume 16bit for other compressed formats
    } else
        mBitDepth = mInputStreamFormat.mBitsPerChannel;
