...
+ (void)initialize
{
    ...
    if (ramSize < 2048) {
        ...
        if (ramSize < 512)
            [defaultValues setObject:[NSNumber numberWithLong:256] forKey:AUDMaxAudioBufferSize];
        else
            [defaultValues setObject:[NSNumber numberWithLong:(NSUInteger)(ramSize / 2)] forKey:AUDMaxAudioBufferSize];
    } else
        [defaultValues setObject:[NSNumber numberWithLong:1024] forKey:AUDMaxAudioBufferSize];
...
- (void)awakeFromNib
{
    ...
    mLCDSelectedStringAttributes = [[NSDictionary alloc] initWithObjects:[NSArray arrayWithObjects:lcdFont,
                                                                             [NSColor colorWithCalibratedRed:29.0f / 350.0f
                                                                                                       green:0.4f
                                                                                                        blue:118.0f / 350.0f
                                                                                                       alpha:1.0f],
                                                                             [NSNumber numberWithFloat:1.0f],
    ...
    [nc addObserver:self
           selector:@selector(handlePlaylistTrackAppended:)
               name:AUDPlaylistItemAppendedtoPlaylistNotification
             object:nil];
    [nc addObserver:self
           selector:@selector(handlePlaylistReplacedLoadedBuffer:)
               name:AUDPlaylistItemInsertedAtLoadedPositionNotification
             object:nil];
    [nc addObserver:self
           selector:@selector(handleNewPlayingTrackSelected:)
               name:AUDPlaylistSelectPlayingTrackNotification
             object:nil];
    [nc addObserver:self
           selector:@selector(handlePlaylistSelectionCursorChanged:)
               name:AUDPlaylistSelectionCursorChangedNotification
             object:nil];
    [nc addObserver:self
           selector:@selector(handleMovePlayingTrackSelected:)
               name:AUDPlaylistMovePlayingTrackNotification
             object:nil];
    [nc addObserver:self
           selector:@selector(handleStartPlaybackNotification:)
               name:AUDStartPlaybackNotification
             object:nil];
    [nc addObserver:self
    ...
    [nc addObserver:self
           selector:@selector(handleDeviceChange:)
               name:AUDPreferredDeviceChangeNotification
             object:nil];
    [nc addObserver:self
           selector:@selector(handleUpdateAppearanceMode:)
               name:AUDAppearanceMode
             object:nil];
    [nc addObserver:self
           selector:@selector(handleUpdateUISkinTheme:)
               name:AUDUISkinTheme
             object:nil];
    [nc addObserver:self
           selector:@selector(handleUpdateAppleRemoteUse:)
               name:AUDAppleRemoteUseChangeNotification
             object:nil];
    [nc addObserver:self
...
- (IBAction)positionSliderMoved:(id)sender
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self
                                             selector:@selector(seekPosition:)
                                               object:sender];
...
- (IBAction)seekPrevious:(id)sender
{
    if ([audioOut isPlaying]) {
        ...
        if ([mPlaylistDoc nonShuffledIndexFromShuffled:[mPlaylistDoc playingTrackIndex]] > 0) {
            NSInteger previousPlaylistPlayingPos = [mPlaylistDoc shuffledIndexFromNonShuffled:
                    [mPlaylistDoc nonShuffledIndexFromShuffled:
                            [mPlaylistDoc playingTrackIndex]]
                - 1];
            [mPlaylistDoc setPlayingTrackIndex:previousPlaylistPlayingPos];
            [mPlaylistDoc setLoadedTrackIndex:previousPlaylistPlayingPos];
            [mPlaylistDoc setLoadedTrackNonShuffledIndex:[mPlaylistDoc nonShuffledIndexFromShuffled:previousPlaylistPlayingPos]];
...
{
    [audioOut setMasterVolumeScalar:[masterDeviceVolume floatValue]
                            forType:[audioOut availableVolumeControls] & kAudioVolumePhysicalControl
            ? kAudioVolumePhysicalControl
            : kAudioVolumeVirtualControl];
}
...
- (void)hidRemote:(HIDRemote*)hidRemote eventWithButton:(HIDRemoteButtonCode)buttonCode isPressed:(BOOL)isPressed fromHardwareWithAttributes:(NSMutableDictionary*)attributes
{
    if (isPressed) {
        ...
        case kHIDRemoteButtonCodeUp: {
            ...
            if (availVolCtrl) {
                [audioOut setMasterVolumeScalar:[masterDeviceVolume floatValue] + (Float32)0.1
                                        forType:availVolCtrl & kAudioVolumePhysicalControl
                        ? kAudioVolumePhysicalControl
                        : kAudioVolumeVirtualControl];
            }
        } break;
                   ...
                   reset:(BOOL)isReset
{
    ...
    if ((bufferLoading != [audioOut playingBuffer])
        && ![audioOut areBothBuffersFromSameFile])
        return;
    ...
    if ((songTotalFrameLength > 0) && (((lastLoadedFrame + firstLoadedFrame) < songTotalFrameLength) || ((firstLoadedFrame > 0) && (!displayBothBuffersStatus || (mAudioBuffersLoadStatus[otherBuffer].firstLoadedFrame > 0))))) {
        ...
        if (lastFrameToLoad != 0) {
            ...
            if (isComplete)
                [[NSColor colorWithCalibratedRed:0.0f green:0.15f blue:0.0f alpha:1.0f] set];
            else
                [[NSColor darkGrayColor] set];
        ...
        if (displayBothBuffersStatus) {
            if (mAudioBuffersLoadStatus[otherBuffer].lastFrameToLoad != 0) {
                ...
                if (mAudioBuffersLoadStatus[otherBuffer].loadCompleted)
                    [[NSColor colorWithCalibratedRed:0.0f green:0.15f blue:0.0f alpha:1.0f] set];
                else
                    [[NSColor darkGrayColor] set];
...
- (void)handleNewPlayingTrackSelected:(NSNotification*)notification
{
    ...
    if (![audioOut isPlaying]) {
        ...
        [self startPlaying];
    } else if (newTrackIndex == [mPlaylistDoc playingTrackIndex])
        return;
        ...
        [self seekNext:nil];
    } else if (newTrackIndex == ([mPlaylistDoc shuffledIndexFromNonShuffled:[mPlaylistDoc nonShuffledIndexFromShuffled:[mPlaylistDoc playingTrackIndex]] - 1])) {
...
- (void)handleMovePlayingTrackSelected:(NSNotification*)notification
{
    ...
    if (newPlayingTrackIndex == -1) {
        ...
        if (![audioOut isPlaying]) {
            ...
            && (newLoadedTrackIndex == [mPlaylistDoc shuffledIndexFromNonShuffled:([mPlaylistDoc nonShuffledIndexFromShuffled:newPlayingTrackIndex] + 1)])
