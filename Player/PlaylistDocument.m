...
- (void)dealloc
{
    ...
    if (mShuffleIndexes) {
        [mShuffleIndexes release];
        mShuffleIndexes = nil;
    }
    if (mInsertTracksDispatchQueue) {
        dispatch_resume(mInsertTracksDispatchQueue);
        mInsertTracksDispatchQueue = NULL;
    }
    [super dealloc];
}
...
- (void)insertPlaylistItems:(NSArray*)urlsToOpen atRow:(NSUInteger)row sortToplist:(BOOL)isSorted
{
    ...
    if (!mInsertTracksDispatchQueue) {
        mInsertTracksDispatchQueue = dispatch_queue_create("com.github.durian.insertTracks", NULL);
    }
    ...
    dispatch_async(mInsertTracksDispatchQueue, ^{
        ...
        if (isSorted) {
            ...
            sortedUrlsToOpen = urlsToOpen;

        for (NSURL* url in sortedUrlsToOpen) {
            if (mAbortAddingTracks)
                break;
...
- (bool)insertPlaylistItem:(NSURL*)itemURL atRow:(NSUInteger)row
{
    ...
    if (fileLoader) {
        ...
        if (str)
            [newItem setAlbum:str];
        str = [fileLoader artist];
        if (str)
            [newItem setArtist:str];
        str = [fileLoader composer];
        if (str)
            [newItem setComposer:str];
...
- (void)movePlaylistItems:(NSIndexSet*)rowsToMove toRow:(NSInteger)rowToInsert
{
    ...
    while (rowIndex != NSNotFound) {
        if (rowIndex == mPlayingTrackIndex)
            playingTrackIndexInSet = posInSet;
        else if (rowIndex == mLoadedTrackIndex)
            loadedTrackIndexInSet = posInSet;
        ...
        if (rowIndex < rowToInsert)
            rowToInsert--;

        if (rowIndex < newPlayingTrackIndex)
            newPlayingTrackIndex--;
        if (rowIndex < newLoadedTrackIndex)
            newLoadedTrackIndex--;
    ...
    if (newPlayingTrackIndex >= rowToInsert)
        newPlayingTrackIndex += [rowsToMove count];
    if (newLoadedTrackIndex >= rowToInsert)
        newLoadedTrackIndex += [rowsToMove count];

    if (playingTrackIndexInSet != NSNotFound)
        newPlayingTrackIndex = playingTrackIndexInSet + rowToInsert;

    if (loadedTrackIndexInSet != NSNotFound)
        newLoadedTrackIndex = loadedTrackIndexInSet + rowToInsert;
...
- (void)removePlaylistItems:(NSIndexSet*)rowsToRemove
{
    ...
    if (newPlayingTrackIndex >= playlistCount)
        newPlayingTrackIndex = playlistCount - 1;
    if (newLoadedTrackIndex < newPlayingTrackIndex)
        newLoadedTrackIndex = newPlayingTrackIndex + 1;
    if (newLoadedTrackIndex >= playlistCount)
        newLoadedTrackIndex = playlistCount - 1;
...
{
    if (!mIsShuffling || (shuffledIndex < 0) || ((NSUInteger)shuffledIndex >= [mShuffleIndexes count]))
        return shuffledIndex;
    else
        return [mShuffleIndexes indexOfObject:[NSNumber numberWithInteger:shuffledIndex]];
}

- (NSInteger)shuffledIndexFromNonShuffled:(NSInteger)nonShuffledIndex
{
    if (!mIsShuffling || (nonShuffledIndex < 0) || ((NSUInteger)nonShuffledIndex >= [mShuffleIndexes count]))
        return nonShuffledIndex;
    else
        return [[mShuffleIndexes objectAtIndex:nonShuffledIndex] longValue];
}
...
{
    if ((mLoadedTrackIndex < 0) || ((NSUInteger)mLoadedTrackIndex >= [playlist count]))
        return nil;
...
{
    if ((index < 0) || ((NSUInteger)index >= [playlist count]))
        return nil;
