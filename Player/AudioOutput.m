...
{
    if (mPhysicalFormats)
        free(mPhysicalFormats);
    mPhysicalFormats = NULL;
    if (mVirtualFormats)
        free(mVirtualFormats);
    mVirtualFormats = NULL;
    [super dealloc];
}
...
- (void)dealloc
{
    if (mAvailableSampleRates)
        free(mAvailableSampleRates);
    if (name)
        [name release];
    if (UID)
        [UID release];
    ...
    void* inClientData)
{
    ...
    if (!bufferData->isSimpleStereoDevice) { // change if more than stereo supported
        ...
        if (bufferData->isIntegerModeOn && bufferData->integerModeStreamFormat.mChannelsPerFrame == 1 && bufferData->integerModeStreamFormat.mBytesPerFrame == 4) {
            ...
            for (unsigned int i = 0; i < framesToCopy; i++) {
                *((SInt32*)outOutputData->mBuffers[bufferData->channelMap[0].stream].mData
                    ...
                    = *((SInt32*)bufferData->buffers[playingBuffer].data + (bufferData->buffers[playingBuffer].currentPlayingFrame * bufferData->buffers[playingBuffer].bytesPerFrame / 4 + 2 * i + 0));
                *((SInt32*)outOutputData->mBuffers[bufferData->channelMap[1].stream].mData
                    ...
                    = *((SInt32*)bufferData->buffers[playingBuffer].data + (bufferData->buffers[playingBuffer].currentPlayingFrame * bufferData->buffers[playingBuffer].bytesPerFrame / 4 + 2 * i + 1));
            }
            ...
            for (UInt32 i = 0; i < framesToCopy; i++) {
                *(UInt16*)((UInt8*)outOutputData->mBuffers[bufferData->channelMap[0].stream].mData
                    + (outOutputData->mBuffers[bufferData->channelMap[0].stream].mNumberChannels * i
                          + bufferData->channelMap[0].channel)
                        * bufferData->buffers[playingBuffer].bytesPerFrame / 2)
                    = *(UInt16*)((UInt8*)bufferData->buffers[playingBuffer].data + (bufferData->buffers[playingBuffer].currentPlayingFrame + i) * bufferData->buffers[playingBuffer].bytesPerFrame);
                *((UInt8*)outOutputData->mBuffers[bufferData->channelMap[0].stream].mData
                    + (outOutputData->mBuffers[bufferData->channelMap[0].stream].mNumberChannels * i
                          + bufferData->channelMap[0].channel)
                        * bufferData->buffers[playingBuffer].bytesPerFrame / 2
                    + 2)
                    = *((UInt8*)bufferData->buffers[playingBuffer].data + (bufferData->buffers[playingBuffer].currentPlayingFrame + i) * bufferData->buffers[playingBuffer].bytesPerFrame + 2);
                ...
                *(UInt16*)((UInt8*)outOutputData->mBuffers[bufferData->channelMap[1].stream].mData
                    + (outOutputData->mBuffers[bufferData->channelMap[1].stream].mNumberChannels * i
                          + bufferData->channelMap[1].channel)
                        * bufferData->buffers[playingBuffer].bytesPerFrame / 2)
                    = *(UInt16*)((UInt8*)bufferData->buffers[playingBuffer].data + (2 * bufferData->buffers[playingBuffer].currentPlayingFrame + 2 * i + 1) * bufferData->buffers[playingBuffer].bytesPerFrame / 2);
                *((UInt8*)outOutputData->mBuffers[bufferData->channelMap[1].stream].mData
                    + (outOutputData->mBuffers[bufferData->channelMap[1].stream].mNumberChannels * i
                          + bufferData->channelMap[1].channel)
                        * bufferData->buffers[playingBuffer].bytesPerFrame / 2
                    + 2)
                    = *((UInt8*)bufferData->buffers[playingBuffer].data + (2 * bufferData->buffers[playingBuffer].currentPlayingFrame + 2 * i + 1) * bufferData->buffers[playingBuffer].bytesPerFrame / 2 + 2);
            }
        } else {
            for (UInt32 i = 0; i < framesToCopy; i++) {
                bcopy(((UInt8*)bufferData->buffers[playingBuffer].data + (bufferData->buffers[playingBuffer].currentPlayingFrame + i) * bufferData->buffers[playingBuffer].bytesPerFrame),
                    ((UInt8*)outOutputData->mBuffers[bufferData->channelMap[0].stream].mData
                        + (outOutputData->mBuffers[bufferData->channelMap[0].stream].mNumberChannels * i
                              + bufferData->channelMap[0].channel)
                            * bufferData->buffers[playingBuffer].bytesPerFrame / 2),
                ...
                bcopy(((UInt8*)bufferData->buffers[playingBuffer].data + (2 * bufferData->buffers[playingBuffer].currentPlayingFrame + 2 * i + 1) * bufferData->buffers[playingBuffer].bytesPerFrame / 2),
                    ((UInt8*)outOutputData->mBuffers[bufferData->channelMap[1].stream].mData
                        + (outOutputData->mBuffers[bufferData->channelMap[1].stream].mNumberChannels * i
                              + bufferData->channelMap[1].channel)
                            * bufferData->buffers[playingBuffer].bytesPerFrame / 2),
    ...
    // Simple stereo device
        ...
        bcopy((char*)bufferData->buffers[playingBuffer].data + (bufferData->buffers[playingBuffer].currentPlayingFrame * bufferData->buffers[playingBuffer].bytesPerFrame),
    ...
    if (bufferSwap) {
        ...
        if (bufferData->buffers[0].sampleRate == bufferData->buffers[1].sampleRate) {
            ...
            bufferData->isIOPaused |= kAudioIOProcSampleRateChanging;
                ...
                if (outSize > 0) {
                    ...
                    if (theBufferList != NULL) {
                        ...
                        if (theNumberOutputChannels > 0) {
                            ...
                            if (result)
                                printf("Error in AudioObjectGetPropertyData getting device name: %d\n", (int)result);
                            else
...
- (BOOL)getIntegerModeFormat:(AudioStreamBasicDescription*)streamFormat forSampleRate:(Float64)splRateToBe
{
    ...
    if (mBufferData.channelMap[0].stream == mBufferData.channelMap[1].stream)
        return [[[[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] streams] objectAtIndex:mBufferData.channelMap[0].stream]
            getIntegerModeFormat:streamFormat
                   forSampleRate:splRateToBe
                  forMinChannels:2];
...
- (BOOL)getPhysicalMixableFormat:(AudioStreamBasicDescription*)streamFormat forSampleRate:(Float64)splRateToBe
{
    ...
    if (mBufferData.channelMap[0].stream == mBufferData.channelMap[1].stream)
        return [[[[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] streams] objectAtIndex:mBufferData.channelMap[0].stream]
            getPhysicalMixableFormat:streamFormat
                       forSampleRate:splRateToBe
                      forMinChannels:2];
    // Handle case of 1 channel multi streams devices
    else
    ...
    if (![[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] isSampleRateHandled:mBufferData.buffers[bufferToFill].sampleRate
        ...
        || (mBufferData.isIntegerModeOn
            && ![[[[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] streams] objectAtIndex:mBufferData.channelMap[0].stream]
                 isSampleRateHandled:mBufferData.buffers[bufferToFill].sampleRate
                withSameStreamFormat:&mBufferData.integerModeStreamFormat])) {
        ...
        if ([[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] isSampleRateHandled:4 * mBufferData.buffers[bufferToFill].sampleRate
            ...
            && (!mBufferData.isIntegerModeOn
                || [[[[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] streams] objectAtIndex:mBufferData.channelMap[0].stream]
                     isSampleRateHandled:4 * mBufferData.buffers[bufferToFill].sampleRate
                    withSameStreamFormat:&mBufferData.integerModeStreamFormat])) {
            ...
            && (!mBufferData.isIntegerModeOn
                || [[[[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] streams] objectAtIndex:mBufferData.channelMap[0].stream]
                     isSampleRateHandled:2 * mBufferData.buffers[bufferToFill].sampleRate
                    withSameStreamFormat:&mBufferData.integerModeStreamFormat])) {
...
- (void)setPlayingBuffer:(int)playingBuffer
{
    if (![self areBothBuffersFromSameFile]) {
        ...
        if (title == nil)
            title = [[mBufferData.buffers[playingBuffer].inputFileLoader inputFileURL] lastPathComponent];
...
- (void)setSamplingRate:(Float64)newSamplingRate
{
    ...
    if (newSamplingRate != audioDeviceCurrentNominalSampleRate) {
        if ([[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] isSampleRateHandled:newSamplingRate
            ...
            if (err == kAudioHardwareNoError)
                audioDeviceCurrentNominalSampleRate = newSamplingRate;
            else
                mBufferData.isIOPaused &= ~kAudioIOProcSampleRateChanging;
        }
    } else
...
- (void)setMasterVolumeScalar:(Float32)newVolume forType:(UInt32)typeOfVolume
{
    ...
    if (newVolume < 0.0)
        newVolume = (Float32)0.0;
    else if (newVolume > 1.0)
        newVolume = (Float32)1.0;
    ...
    case kAudioVolumeVirtualControl:
        if ([[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] availableVolumeControls] & kAudioVolumeVirtualControl) {
            ...
            if (currentVolume <= 0)
                ...
                currentVolume = (Float32)(20.0 * log(currentVolume));
        }
        break;
    default:
        return (Float32)0.0;
...
- (bool)initiatePlaybackAfterHoggingDevice
{
    ...
    [[self class] cancelPreviousPerformRequestsWithTarget:self
                                                 selector:@selector(initiatePlaybackTimedOut)
                                                   object:nil];
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:AUDIntegerMode]
        ...
        audioDeviceCurrentPhysicalBitDepth = audioFormat.mBitsPerChannel;
        if (audioDeviceCurrentPhysicalBitDepth > 24)
            audioDeviceCurrentPhysicalBitDepth = 24; // 32bit float can't convey more than 24bits
...
- (bool)stop
{
    ...
    [mBufferData.appController updateMetadataDisplay:NSLocalizedString(@"Stopping audio device...", @"Info line stopping device")
                                               album:@""
                                              artist:@""
                                            composer:@""
                                          coverImage:nil
...
- (void)completeDeviceStop
{
    ...
    // Cancel timeout
    [[self class] cancelPreviousPerformRequestsWithTarget:self
                                                 selector:@selector(completeDeviceStop)
                                                   object:nil];
    ...
    deviceMaxSplRate = [[audioDevicesList objectAtIndex:selectedAudioDeviceIndex] maxSampleRate];
    [mBufferData.appController updateMetadataDisplay:@""
                                               album:@""
                                              artist:@""
                                            composer:@""
                                          coverImage:nil
